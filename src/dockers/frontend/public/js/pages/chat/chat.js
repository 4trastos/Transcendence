...
`;
    ...
    initEvents() {
        ...
        this.socket = new WebSocket("wss://transcendence.42.fr/api/v1/chats/connect-ws?userId=3");
    ...
    async loadChats() {
        if (this.element === null)
            return;
        try {
            const res = await fetch("https://transcendence.42.fr/api/v1/chats/user/3", {
                method: "GET",
                credentials: "include",
    ...
    renderChatList(chats) {
        ...
        chats.forEach((chat) => {
            ...
            const chatTitle = chat.isGroupChat
                ? chat.title
                : chat.users.filter((str) => str !== "3").join(", ");
            ...
            const chatItemComponent = new ChatItem({
                ...
                lastMessage: chat.messages[chat.messages.length - 1]?.content.text || "", // Último mensaje
    ...
    async openChat(chatId) {
        if (this.element === null)
            return;
        try {
            const res = await fetch(`https://transcendence.42.fr/api/v1/chats/${chatId}/messages`, {
                method: "GET",
                credentials: "include",
                headers: { "Content-Type": "application/json" },
            });
