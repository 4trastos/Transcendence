import { RankCard } from "../../components/Card/RankCard.js";
import { ChartComponent } from "../../components/Charts/ChartComponent.js";
import { CircularProgressCardComponent } from "../../components/CircularProgressCard/CircularProgressCardComponent.js";
...
export class StatsPage extends Component {
    ...
    async initEvents() {
        if (!this.element)
            return;
        const centerStats = this.element.querySelector("#center-stats");
        if (centerStats) {
            const chart = new ChartComponent({});
            centerStats.appendChild(chart.render());
        }
        const leftStats = this.element.querySelector("#left-stats");
        if (leftStats) {
        ...
        const rigthStats = this.element.querySelector("#rigth-stats");
        if (rigthStats) {
