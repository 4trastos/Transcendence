...
export class Navigation extends Component {
    ...
    changeActiveItem(newActiveItem) {
        const underline = this.element?.querySelector('#active-underline');
        const navContainer = this.element?.querySelector('#ul-items');
        this.props.items.forEach((item) => {
            const itemId = `#item-${item.text.toLowerCase()}`;
            const itemElement = this.element?.querySelector(itemId);
