...
export class FloatingChatComponent extends Component {
    ...
    updateStatus() {
        const statusElement = this.element?.querySelector(`#${this.props.id}-status`);
        let nActive = 0;
        this.props.onlineUser?.forEach((status, userId) => {
            nActive += status ? 1 : 0;
        });
    ...
    renderTemplate() {
        return `
	<div id="${this.props.id}" class=" backdrop-3xl bg-opacity-15 bg-[#1D1F2B] bottom-4 right-4 px-5 w-[18rem] h-[22rem]  border border-white border-opacity-15 rounded-2xl shadow-lg flex flex-col overflow-hidden z-50">
	<!-- Header del chat -->	
...
<form id="${this.props.id}-form" class=" m-4 rounded-lg flex items-center bg-[#1D1F2B] bg-opacity-30">
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
    ...
    initEvents() {
        ...
        if (this.element) {
            const content = this.element.querySelector(`#${this.props.id}-body`);
            this.updateStatus();
            let count = 0;
        ...
        const header = this.element?.querySelector(`#${this.props.id}-header`);
        if (!header)
            return;
        header.addEventListener("click", () => {
            const chatItem = header.closest(`#${this.props.id}`);
            if (!chatItem)
                return;
...
//# sourceMappingURL=FloatingChatComponent.js.map