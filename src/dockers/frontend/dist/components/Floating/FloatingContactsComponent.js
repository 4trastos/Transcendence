...
class ChatItemComponent extends Component {
    ...
    updateStatus(online) {
        const itemStatus = this.element?.querySelector('#chat-item-status');
        if (!itemStatus)
            return;
...
export default class FloatingChatListComponent extends Component {
    ...
    changeStatus(chatId, newStatus) {
        const chatItem = this.element?.querySelector(`#chat-${chatId}`);
        if (!chatItem)
            return;
        const status = chatItem.querySelector('#chat-item-status');
        if (!status)
            return; //Si es un chat grupal no hay status
  ...
  <div id="chat-list" class="h-fit w-full overflow-y-auto mb-4 rounded-2xl bg-white bg-opacity-10">
    ...
    initEvents() {
        ...
        header.addEventListener("click", () => {
            const chatItem = header.closest(`#contacts`);
            if (!chatItem)
                return;
...
//# sourceMappingURL=FloatingContactsComponent.js.map